...
volatile uint8_t device_address = 0x1A; // 0x1A
...
uint8_t QMC_init(QMC_t *qmc,I2C_HandleTypeDef *i2c,uint8_t Output_Data_Rate)
{
	...
	if(HAL_I2C_Mem_Write(qmc->i2c, device_address, 0x0B, 1, &array[0], 1, 100)!=HAL_OK)return 1;
	if(HAL_I2C_Mem_Write(qmc->i2c, device_address, 0x09, 1, &array[1], 1, 100)!=HAL_OK)return 1;
...
{
	  qmc->datas[0]=0;
	  HAL_I2C_Mem_Read(qmc->i2c, device_address, 0x06, 1, qmc->datas, 1, 100);

	  if((qmc->datas[0]&0x01)==1)
	  {
		  HAL_I2C_Mem_Read(qmc->i2c, device_address, 0x00, 1, qmc->datas, 6, 100);
...
{
	uint8_t array[1]={0};
	if(HAL_I2C_Mem_Write(qmc->i2c, device_address, 0x09, 1, &array[0], 1, 100)!=HAL_OK)return 1;
	return 0;
}
uint8_t QMC_Reset(QMC_t *qmc)
{
	uint8_t array[1]={0x80};
	if(HAL_I2C_Mem_Write(qmc->i2c, device_address, 0x0A, 1, &array[0], 1, 100)!=HAL_OK)return 1;
	return 0;
}
